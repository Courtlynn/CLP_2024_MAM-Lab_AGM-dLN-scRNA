...
### ISG-stimmed T cells########
#############################
setwd("~/Desktop/R work/No Dual_scRNA Sequencing Project - CLP and JMG copy/DEGs/ISG-stimulated cells ")
Idents(seurat) <- "celltype.stim"
ISG_MvP <- FindMarkers(seurat, ident.1 = "ISG-stimulated cells_M229", ident.2 = "ISG-stimulated cells_PBS", verbose = FALSE)
write.csv(ISG_MvP,file='ISG_M229vPBS.csv',quote=F)
## Remove Y linked genes ##
filtered_ISG_MvP_genes <- ISG_MvP[!rownames(ISG_MvP) %in% y_hgnc_symbol, ]
##Remove polymorphic HLA-II genes##
final_ISG_MvP_genes <- filtered_ISG_MvP_genes[!rownames(filtered_ISG_MvP_genes) %in% MHCII, ]
write.csv(final_ISG_MvP_genes,file='NoY_noMHCII_ISG_M229vPBS.csv',quote=F)
...
dfg
df
final_ISG_MvP_1 <- left_join(dfg,df,by=c("RowNames"="V2"))
View(final_MvP_1)
...
final_ISG_MvP_1 <- final_ISG_MvP_1[final_ISG_MvP_1$p_val_adj < 0.05,]
final_ISG_MvP_2 <- final_ISG_MvP_1[final_ISG_MvP_1$avg_log2FC > 0,]
gene_list_up = final_ISG_MvP_2$avg_log2FC
names(gene_list_up) = final_ISG_MvP_2$V1
...
set.seed(1234)
GO_ISG_MvP_up<- enrichGO(names(gene_list_up), 'org.Hs.eg.db', ont="BP", keyType = "ENTREZID", pvalueCutoff=0.01) 
GO_ISG_MvP_up_1 <- clusterProfiler:: simplify(GO_ISG_MvP_up, cutoff=0.7, by="p.adjust", select_fun=min)
dotplt_ISG_MvP_up_1 <- dotplot(GO_ISG_MvP_up_1)+ ggtitle(label = "Up_GO_Enrichment: IPR8+m229 v PBS", subtitle = "ISG-stimulated T cells")
dotplt_ISG_MvP_up_1 ##Export Plot as PNG 
...
final_ISG_MvP_1 <- final_ISG_MvP_1[final_ISG_MvP_1$p_val_adj < 0.05,]
final_ISG_MvP_3 <- final_ISG_MvP_1[final_ISG_MvP_1$avg_log2FC < 0,]
gene_list_dwn = final_ISG_MvP_3$avg_log2FC
names(gene_list_dwn) = final_ISG_MvP_3$V1
...
set.seed(1234)
GO_ISG_MvP_dwn<- enrichGO(names(gene_list_dwn), 'org.Hs.eg.db', ont="BP", keyType = "ENTREZID", pvalueCutoff=0.01) 
GO_ISG_MvP_dwn_1 <- clusterProfiler:: simplify(GO_ISG_MvP_dwn, cutoff=0.7, by="p.adjust", select_fun=min)
dotplt_ISG_MvP_dwn_1 <- dotplot(GO_ISG_MvP_dwn_1)+ ggtitle(label = "Down_GO_Enrichment: IPR8+m229 v PBS", subtitle = "ISG-stimulated T cells")
dotplt_ISG_MvP_dwn_1 ##Export Plot as PNG
