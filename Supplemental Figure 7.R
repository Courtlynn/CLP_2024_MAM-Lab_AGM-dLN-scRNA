...
DotPlot(seurat, features = rownames(Btop25_FvM), scale = T, dot.scale = 10, group.by = "condition" ,idents = "B cells")+
  theme_minimal() + ggtitle(label = "B cells: IPR8+flg v IPR8+m229", subtitle = "Top 25 most signifcant DEG (Log2FC > 0)") + # Change the theme to minimal and add title 
...
Idents(seurat) <- seurat@meta.data$celltype
DotPlot(seurat, features = rownames(Bbot25_FvM), scale = T, dot.scale = 10, group.by = "condition", idents = "B cells")+
  theme_minimal() + ggtitle(label = "B cells: IPR8+flg v IPR8+m229", subtitle = "Top 25 most signifcant DEG (Log2FC < 0)") +  # Change the theme to minimal
  theme(plot.background = element_rect(fill = "white")) +  # Set plot background to white
  theme(axis.text.x = element_text(size = 14, angle = 45, hjust = 1, margin = margin(t = 10))) +  # Rotate x-axis labels by 45 degrees and adjust horizontal justification
  scale_colour_gradient2(low="black", mid="lightgrey", high="darkgoldenrod")
...
Idents(seurat) <- seurat@meta.data$celltype
DotPlot(seurat, features = rownames(Btop25_RvM), scale = T, dot.scale = 10, group.by = "condition" ,idents = "B cells")+
  theme_minimal() + ggtitle(label = "B cells: IPR8-R848 v IPR8+m229", subtitle = "Top 25 most signifcant DEG (Log2FC > 0)") + # Change the theme to minimal and add title 
  theme(plot.background = element_rect(fill = "white")) +  # Set plot background to white
  theme(axis.text.x = element_text(size = 14, angle = 45, hjust = 1, margin = margin(t = 10))) +  # Rotate x-axis labels by 45 degrees and adjust horizontal justification
  scale_colour_gradient2(low="black", mid="lightgrey", high="darkgoldenrod")

##Top 25 most significantly downregulated genes##
Bbot25_RvM <- final_B_RvM_genes %>%  
  dplyr::filter(p_val_adj < 0.05) %>%
  dplyr:: filter(avg_log2FC < -0) %>%
  dplyr:: arrange(p_val_adj)%>%
  slice_head(n = 25) %>%
...
Idents(seurat) <- seurat@meta.data$celltype
DotPlot(seurat, features = rownames(Bbot25_RvM), scale = T, dot.scale = 10, group.by = "condition", idents = "B cells")+
  theme_minimal() + ggtitle(label = "B cells: IPR8-R848 v IPR8+m229", subtitle = "Top 25 most signifcant DEG (Log2FC < 0)") +  # Change the theme to minimal
  theme(plot.background = element_rect(fill = "white")) +  # Set plot background to white
  theme(axis.text.x = element_text(size = 14, angle = 45, hjust = 1, margin = margin(t = 10))) +  # Rotate x-axis labels by 45 degrees and adjust horizontal justification
  scale_colour_gradient2(low="black", mid="lightgrey", high="darkgoldenrod")
